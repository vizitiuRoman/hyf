...
func (s *grpcServer) Register(descriptors []*ServerDescriptor) error {
	for _, descriptor := range descriptors {
		...
		err := descriptor.GRPCGatewayRegistrar(s.ctx, s.mux, fmt.Sprintf("%s:%d", s.cfg.Host, s.cfg.GrpcPort), s.opts)
		if err != nil {
			return err
		}
	}
